#!/usr/bin/env python3
"""
Apply bug fixes to the backtest system files.
"""
...
def fix_signal_generator():
    """Fix the deprecated fillna method in signal_generator.py"""
    file_path = Path("crypto_momentum_backtest/signals/signal_generator.py")
    
    if file_path.exists():
        with open(file_path, 'r') as f:
            content = f.read()
        
        # Fix the deprecated fillna method
        old_line = "df['position'] = df['position'].replace(0, np.nan).fillna(method='ffill').fillna(0)"
        new_line = "df['position'] = df['position'].replace(0, np.nan).ffill().fillna(0)"
        ...
        print("✅ Fixed signal_generator.py")


def fix_rebalancer():
    """Fix the deprecated frequency string in rebalancer.py"""
    file_path = Path("crypto_momentum_backtest/portfolio/rebalancer.py")
    
    if file_path.exists():
        with open(file_path, 'r') as f:
            content = f.read()
        
        # Fix the deprecated frequency strings
        content = content.replace("'M'", "'ME'")
        content = content.replace("'Q'", "'QE'")
        
        with open(file_path, 'w') as f:
            f.write(content)
        
        print("✅ Fixed rebalancer.py")


def fix_position_sizer():
    """Fix the Series/DataFrame issue in position_sizer.py"""
    file_path = Path("crypto_momentum_backtest/portfolio/position_sizer.py")
    
    if file_path.exists():
        # Write the fixed version
        fixed_content = '''"""Position sizing based on ATR and risk management."""
import numpy as np
import pandas as pd
...
'''
        ...
        print("✅ Fixed position_sizer.py")


def fix_backtest_engine():
    """Fix the backtest engine to pass data correctly"""
    file_path = Path("crypto_momentum_backtest/backtest/engine.py")
    
    if file_path.exists():
        with open(file_path, 'r') as f:
            content = f.read()
        
        # Fix the position sizing call
        old_code = """position_sizes = self.position_sizer.calculate_position_sizes(
        ...
        new_code = """# Ensure we have the right data format
                if i < len(atr_data):
...
def main():
    ...
    print("\n✅ All fixes applied!")
    print("\nYou can now run the backtest again:")
    print("python run.py")
